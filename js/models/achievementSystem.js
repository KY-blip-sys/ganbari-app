...
export const ACHIEVEMENT_LIST = [
  // ---------- 累計EXP ----------
  { id: "exp-10", icon: "star", name: "初めの一歩", description: "累計10EXP達成", check: (ctx) => ctx.totalExp >= 10 },
  { id: "exp-100", icon: "auto_awesome", name: "軌道に乗ってきた", description: "累計100EXP達成", check: (ctx) => ctx.totalExp >= 100 },
  { id: "exp-500", icon: "local_fire_department", name: "継続は力なり", description: "累計500EXP達成", check: (ctx) => ctx.totalExp >= 500 },
  { id: "exp-1000", icon: "diamond", name: "千里の道も一歩から", description: "累計1000EXP達成", check: (ctx) => ctx.totalExp >= 1000 },
  { id: "exp-3000", icon: "stars", name: "止まらない成長", description: "累計3000EXP達成", check: (ctx) => ctx.totalExp >= 3000 },
  { id: "exp-5000", icon: "workspace_premium", name: "伝説の始まり", description: "累計5000EXP達成", check: (ctx) => ctx.totalExp >= 5000 },
  { id: "exp-10000", icon: "military_tech", name: "一万の頂", description: "累計10000EXP達成", check: (ctx) => ctx.totalExp >= 10000 },
  { id: "exp-20000", icon: "nights_stay", name: "限界突破者", description: "累計20000EXP達成", check: (ctx) => ctx.totalExp >= 20000 },
  { id: "exp-50000", icon: "public", name: "生きる伝説", description: "累計50000EXP達成", check: (ctx) => ctx.totalExp >= 50000 },

  // ---------- 記録数 ----------
  { id: "record-1", icon: "edit_note", name: "最初の記録", description: "記録を1件追加する", check: (ctx) => ctx.recordCount >= 1 },
  { id: "record-10", icon: "edit", name: "書き続ける人", description: "記録を10件追加する", check: (ctx) => ctx.recordCount >= 10 },
  { id: "record-50", icon: "menu_book", name: "習慣化への道", description: "記録を50件追加する", check: (ctx) => ctx.recordCount >= 50 },
  { id: "record-100", icon: "auto_stories", name: "百戦錬磨", description: "記録を100件追加する", check: (ctx) => ctx.recordCount >= 100 },
  { id: "record-200", icon: "emoji_events", name: "記録の達人", description: "記録を200件追加する", check: (ctx) => ctx.recordCount >= 200 },
  { id: "record-500", icon: "folder", name: "積み重ねの化身", description: "記録を500件追加する", check: (ctx) => ctx.recordCount >= 500 },
  { id: "record-1000", icon: "inventory_2", name: "記録の殿堂", description: "記録を1000件追加する", check: (ctx) => ctx.recordCount >= 1000 },

  // ---------- 連続記録日数 ----------
  { id: "streak-3", icon: "local_fire_department", name: "3日坊主卒業", description: "3日連続で記録する", check: (ctx) => ctx.maxStreak >= 3 },
  { id: "streak-7", icon: "local_fire_department", name: "週間戦士", description: "7日連続で記録する", check: (ctx) => ctx.maxStreak >= 7 },
  { id: "streak-14", icon: "local_fire_department", name: "2週間の炎", description: "14日連続で記録する", check: (ctx) => ctx.maxStreak >= 14 },
  { id: "streak-30", icon: "local_fire_department", name: "継続の鬼", description: "30日連続で記録する", check: (ctx) => ctx.maxStreak >= 30 },
  { id: "streak-60", icon: "local_fire_department", name: "習慣の化身", description: "60日連続で記録する", check: (ctx) => ctx.maxStreak >= 60 },
  { id: "streak-100", icon: "local_fire_department", name: "百日不滅", description: "100日連続で記録する", check: (ctx) => ctx.maxStreak >= 100 },

  // ---------- 記録した日数（連続でなくても可） ----------
  { id: "days-10", icon: "calendar_month", name: "10日間の軌跡", description: "記録した日数が合計10日", check: (ctx) => ctx.daysRecorded >= 10 },
  { id: "days-30", icon: "event", name: "1ヶ月分の積み上げ", description: "記録した日数が合計30日", check: (ctx) => ctx.daysRecorded >= 30 },
  { id: "days-100", icon: "date_range", name: "百日の足跡", description: "記録した日数が合計100日", check: (ctx) => ctx.daysRecorded >= 100 },
  { id: "days-365", icon: "celebration", name: "一年間の物語", description: "記録した日数が合計365日", check: (ctx) => ctx.daysRecorded >= 365 },
  ...
  // ---------- ベストな一日 ----------
  { id: "best-day-100", icon: "partly_cloudy_day", name: "充実の一日", description: "1日の合計が100EXPを超える", check: (ctx) => ctx.bestDayExp >= 100 },
  { id: "best-day-200", icon: "wb_sunny", name: "全力の一日", description: "1日の合計が200EXPを超える", check: (ctx) => ctx.bestDayExp >= 200 },
  { id: "best-day-300", icon: "star", name: "限界突破の一日", description: "1日の合計が300EXPを超える", check: (ctx) => ctx.bestDayExp >= 300 },

  // ---------- 生活リズム ----------
  { id: "early-bird", icon: "wb_twilight", name: "早起きは三文の徳", description: "朝7時前に5回記録する", check: (ctx) => ctx.earlyBirdCount >= 5 },
  { id: "night-owl", icon: "bedtime", name: "夜型の探求者", description: "夜23時以降に5回記録する", check: (ctx) => ctx.nightOwlCount >= 5 },
