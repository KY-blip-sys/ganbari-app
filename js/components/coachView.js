...
export function renderCoach({ dailyMessage, weakestStat, weakestStatHint, nextAchievement }) {
  containerEl.innerHTML = `
    <section class="card glass-card coach-tip-card">
      <p class="card-label"><span class="card-label-icon">✨</span>今日のひとこと</p>
      <p class="coach-tip-text">${dailyMessage}</p>
    </section>
    <section class="card glass-card coach-tip-card">
      <p class="card-label"><span class="card-label-icon">📊</span>伸ばすと良いステータス</p>
    ...
    <section class="card glass-card coach-tip-card">
      <p class="card-label"><span class="card-label-icon">🏆</span>次に狙える実績</p>
      <p class="coach-tip-text">${
